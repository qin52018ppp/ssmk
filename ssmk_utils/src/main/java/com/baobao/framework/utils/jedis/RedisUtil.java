...
public class RedisUtil {
	 ...
	 * void
	 */
	public String setSerializeKey(final String key, final Object data) {
		return new Executor<String>(shardedJedisPool) {
			@Override
			String execute() {
				return jedis.set(key.getBytes(), SerializeUtil.serialize(data));
			}
		}.getResult();
	}
	/**
	 ...
	 * void
	 */
	public String setSerializeKey(final String key, final Object data, final int expire) {
		return new Executor<String>(shardedJedisPool) {
			@Override
			String execute() {
				return jedis.setex(key.getBytes(), expire, SerializeUtil.serialize(data));
			}
		}.getResult();
	}

	/**
	 * 获取redis缓存中的key值，将其反序列化
	 * @param key
	 * Object
	 */
	public Object getUnserializeKey(final String key) {

		return new Executor<Object>(shardedJedisPool) {
			@Override
			Object execute() {
				if(jedis.exists(key.getBytes())) {
					byte[] bannerBytes= jedis.get(key.getBytes());
					Object obj = SerializeUtil.unserialize(bannerBytes);
					return obj;
				}
				return null;
			}
	...
	public long hincrBy(final String key,final String field, final long value) {
		return ((Long)(new RedisUtil.Executor(this.shardedJedisPool) {
			Long execute() {
				long id = this.jedis.hincrBy(key,field,value);
