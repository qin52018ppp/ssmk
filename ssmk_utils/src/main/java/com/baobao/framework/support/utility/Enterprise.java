...
public class Enterprise<A, B> extends LinkedHashMap<A, B> implements Serializable {
    ...
    public Integer getInt(Object key) {
        ...
        if (value == null || Validation.isEmpty(value.toString()) || value.toString().equals("null")) {
            return 0;
        }
    ...
    public String getString(Object key, String defaultValue) {
        ...
        return Validation.isNotEmpty(value) ? value : defaultValue;
    }
        ...
        try {
            if (value instanceof Date) {
                ...
                if (dateString.equalsIgnoreCase("") || dateString.equals("null")) {
                    ...
                    return DateUtility.parse((String) value);
                }
            }
            return null;
        } catch (Exception ex) {
    ...
    public Enterprise getEntity(String key) {
        ...
        if (value instanceof List) {
            return ((List<Enterprise>) value).get(0);
        } else {
    ...
    public Date getStartDate() {
        Date startDate = DateUtility.parse(DateUtility.FORMAT_SIMPLE, getString("startDate"));
    ...
    public Date getEndDate() {
        Date endDate = DateUtility.parse(DateUtility.FORMAT_SIMPLE, getString("endDate"));
        Calendar calendar = Calendar.getInstance();
    ...
    public boolean isMobile(String field) {
        return Validation.isMobile(getString(field));
    }
    ...
    public boolean isUrl(String key) {
        return Validation.isUrl(getString(key));
    }

    public boolean isDate(String key) {
        return Validation.isDate(get(key));
    }
