...
public class TransportClientTest {
    ...
    public void queryData() {
        ...
        String type="test001";
        //多条件设置
        MatchPhraseQueryBuilder mpq1 = QueryBuilders .matchPhraseQuery("keyWords","重疾");
        MatchPhraseQueryBuilder mpq2 = QueryBuilders .matchPhraseQuery("id","2");
        QueryBuilder qb2 = QueryBuilders.boolQuery().must(mpq1)
                .must(mpq2)
                ;
        ...
        SearchResponse myresponse=responsebuilder.setQuery(qb2).setFrom(0).setSize(20).addSort("id", SortOrder.DESC).setExplain(true).execute().actionGet();
        SearchHits hits = myresponse.getHits();
        for(int i = 0; i < hits.getHits().length; i++) {
            LOGGER.info(hits.getHits()[i].getSourceAsString());
        }

       /* BulkRequestBuilder bulkRequest = client.prepareBulk();
        QueryBuilder queryBuilder = QueryBuilders.matchQuery("keyWords", "重疾");
        SearchResponse response = client.prepareSearch(index).setQuery(queryBuilder).get();
        List<FileInfo> fileInfos=new ArrayList<>();
        for (SearchHit searchHit : response.getHits()) {
            String source=searchHit.getSourceAsString();
            fileInfos.add(new Gson().fromJson(source,FileInfo.class));
        }
        LOGGER.info("====================================ES执行查询{}，结果：{}",index,  new Gson().toJson(fileInfos));*/
    }
   /* @Test
    public void insertData() {
        ...
        for (long i = 2; i < 100 ; i++) {
            ...
            fileInfoObj.setId(i);
            fileInfoObj.setFileName("文件啊实打实"+i);
            fileInfoObj.setFileType("txt");
            fileInfoObj.setKeyWords("重疾"+i);
            fileInfoObj.setOsPath("/txt");
            fileInfoObj.setCreateDate(new Date());
            fileInfoObj.setRemark("asddd");
            fileInfos.add(fileInfoObj);
        }
        for (FileInfo fileInfo : fileInfos) {
            try {
                IndexRequestBuilder indexRequestBuilder = client.prepareIndex(index, type, fileInfo.getId().toString());
                indexRequestBuilder.setSource(XContentFactory.jsonBuilder()
                        .startObject()
                        .field("id",fileInfo.getId())
                        .field("fileName", fileInfo.getFileName())
                        .field("fileType", fileInfo.getFileType())
                        .field("keyWords", fileInfo.getKeyWords())
                        .field("osPath", fileInfo.getOsPath())
                        .field("createDate", DateUtil.getFormatDateTime(fileInfo.getCreateDate(),"yyyy-MM-dd HH:mm:ss"))
                        .field("remark", fileInfo.getRemark())
                        .endObject());
        ...
        LOGGER.info("ES中初始化创建{},导入数据结果：{}", index, response.status());
    }*/
    @Test
...
//            return new Result(res.index());
        } catch (IOException e) {
            LOGGER.info("create mapping error.... ",e);
        }
//        return new Result(RestFulApiExceptionEnum.REST_FUL_API_ES_CLIENT_EXCEPTION);
    }
    ...
    public void addIndex() {
        try {
            IndexResponse response = client.prepareIndex("msg", "tweet", "1").setSource(XContentFactory.jsonBuilder().startObject()
                    .field("userName", "张三")
                    .field("sendDate", new Date())
                    .field("msg", "你好李四")
                    .endObject()).get();
            LOGGER.info("索引名称:{},类型:{},文档ID:{},当前实例状态:{}", response.getIndex(), response.getType(), response.getId(), response.status());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     */
    @Test
    public void addIndexByObject() {
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("userName", "张三123");
        jsonObject.addProperty("sendDate", "2017-11-23");
        jsonObject.addProperty("msg", "你好李四123123");
    ...
    public void autoBulkProcessor() throws Exception {
        ...
        BulkProcessor bulkProcessor = BulkProcessor.builder(client,
                new BulkProcessor.Listener() {
                    ...
                    public void afterBulk(long executionId,
                        ...
                        //提交结束后调用（无论成功或失败）
                        System.out.println("提交" + response.getItems().length + "个文档，用时" + response.getTook() + "MS" + (response.hasFailures() ? " 有文档提交失败！" : ""));
                ...
                .setBulkActions(5)//文档数量达到1000时提交
        ...
        for (int i = 1; i <23 ; i++) {
            ...
            bulkProcessor.add(new IndexRequest("article", "content", "" + i).source(json,XContentType.JSON));//添加文档，以便自动提交
    ...
    public void multipleBulkProcessor() throws Exception {
        ...
        for (int i = 500; i < 1000; i++) {
            ...
            IndexRequestBuilder indexRequest = client.prepareIndex("article", "content")
                    //指定不重复的ID
                    .setSource(jsons,XContentType.JSON).setId(String.valueOf(i));
            //添加到builder中
...
//        }  
    ...
    public void spanFirstQuery() {
        ...
        QueryBuilder queryBuilder = QueryBuilders.spanFirstQuery(
                QueryBuilders.spanTermQuery("title", "C"), 1 // Query
                // Max查询范围的结束位置
        );
        SearchResponse response = client.prepareSearch(article).setQuery(queryBuilder).get();
        for (SearchHit searchHit : response.getHits()) {
    ...
    public void Aggregation() {
        ...
        AggregationBuilder aggregationBuilder = AggregationBuilders
                ...
                .terms("").field("")
                .order(BucketOrder.aggregation("", false))
                .size(10)
...
//      AggregationBuilders.reverseNested("res_negsted").path("kps ");    
    ...
    public void complexSearch1() {
        ...
        if (keyword != null && !keyword.equals("")) {
            QueryBuilder nameBuilder = QueryBuilders.matchQuery("title", keyword).analyzer("ik_max_word").boost(10);
            QueryBuilder labelBuilder = QueryBuilders.matchQuery("content", keyword).analyzer("ik_max_word").boost(10);
//            QueryBuilder categoryBuilder = QueryBuilders.matchQuery("categoryName", keyword).analyzer("ik_max_word").boost(10);
    ...
    public void IKAnalyzer() throws IOException {
        ...
        TokenStream tokenStream = analyzer.tokenStream("ik_", stringReader);
        tokenStream.reset();
    ...
    public static void println(SearchResponse response) {
        ...
        System.err.println(
                "getFailedShards : " + response.getFailedShards() + "\n" +
                        ...
                        "getTookInMillis : " + response.getTook() + "\n" +
